...
\begin{bnf}
  \gd{lexprogram} \eq \gmany{\gp{whitespace}} \ \gmany{ \ \gp{token} \ \gp{whitespace} \ } \\
  \gd{token}      \eq  \gp{keyword}
                  \gor \gp{literal}
                  \gor \gp{operator}
                  \gor \gp{qid}
\end{bnf}
...
\begin{bnf}
  ...
  \\
  \gd{decdig} \eq 0 \gor 1 \gor ... \gor 9 \\
  \gd{bindig} \eq 0 \gor 1 \\
  \gd{octdig} \eq 0 \gor 1 \gor ... \gor 7 \\
  \gd{hexdig} \eq  0 \gor 1 \gor ... \gor 9
...
\begin{bnf}
  ...
  \gd{qid} \eq \gtry{\gp{id} \ \term{:}} \ \gp{id} \\
  \\
  \gd{keyword} \eq
               \term{abstract}
        \gor   \term{and}
...
\begin{bnf}
  ...
  \gd{comment}    \eq \# \ \gmany{\gp{noneol}} \ \gp{eol}
\end{bnf}
...
\begin{bnf}
  \gd{literal} \eq \gp{integer} \gor \gp{float} \gor \gp{string}
\end{bnf}
