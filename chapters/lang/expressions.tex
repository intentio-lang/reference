...
A \emph{statement}\index{statement} is either an assignment or expression. Statements serve mostly to contain and explicitly sequence expression evaluation, forming \emph{blocks}. Evaluating \emph{expression statement}\index{statement!expression statement} means evaluating contained expression.
...
\begin{bnf}
  ...
  \gd{parenblock} \eq \term{(} \ \gp{blockbody} \ \term{)} \\
  \\
  \gd{blockbody} \eq \gmany{ \ \gp{stmt} \ \term{;} \ } \ \gp{expr} \ \gtry{\term{;}}
\end{bnf}

A \emph{block expression}\index{expression!block expression} is a sequence of statements, followed by an expression. Each block introduces a new namespace scope, this means that variables introduced within block are in scope for only the block itself.
...
Characters are compared using their byte representation. Strings and regular expressions are compared character by character.

TODO(Ania): what term is returned?

\subsection{Result operators}

TODO(Ania): not, and, or
