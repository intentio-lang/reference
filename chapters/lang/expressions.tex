...
Literals in Intentio are always immutable, it is not possible to change value of the literal in-place.
...
A \emph{block expression}\index{expression!block expression} is an optional sequence of statements, followed by an expression. Each block introduces a new namespace scope, this means that variables introduced within block are in scope for only the block itself.

Parenblock evaluates statements sequentially while each statement evaluates successfully. On first failing statement, block stops evaluation and its result is equal to the result of failing statement. Otherwise, if all statements succeeded, block evaluates to result of the last expression. If block is empty, its result is a succeeded \emph{unit} term.

Braceblock also evaluates statements sequentially, but it do not stop to evaluate statements even if some of them fails. Its result is always equal to to the result of last expression.
...
No matter what comparsion was made, its result consist term copied from the second arggument.

\subsection{Result operators}

Result operators are used to made logical expressions, such as conjunction, alternative, xor and negation. Their actions are analogous to those from which they took their name in classical mathematics.

The following tables present the exact results for each set of function arguments. The descriptions in the left column correspond to the first argument, while those in the top column - to the second. The results term is copied from second argument.
...
For \emph{not} operator the result has the same term as argument, but its Succ is changed to Fail and vice versa.
...
The variables definied in condition could be used in ifbraceblock and elsebraceblock as it was the same scope.
